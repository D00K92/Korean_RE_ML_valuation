...
from data_pipeline.ingestion.geocode import KEYWORD, PARCEL, ROAD, Coord, GeocodeCache
from data_pipeline.ingestion.geocoding import (
    AddressIndex,
    base_name,
    build_borrow_index,
    geocode_label,
    ...
    df = pd.DataFrame(
        {
            ...
            "dong": ["교남동"],
            "jibun": ["BL-3"],
            "complex_name": ["경희궁자이(3BL)"],
        }
    )
    borrow = {("경희궁자이", "11110"): Coord(37.55, 126.97)}
    client = FakeClient({(KEYWORD, "서울특별시 종로구 경희궁자이"): Coord(1, 1)})
    out = geocode_label(
        df, client, GeocodeCache(path=tmp_path / "c.parquet"), borrow_index=borrow, index=idx
    )
