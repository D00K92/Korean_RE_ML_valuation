...
def extract(regions: list[str] | None = None, mode: str = "backfill") -> pd.DataFrame:
    """Fetch raw commercial POIs for `regions` (default = full configured scope).
    ...
    `mode`:
      - "backfill" (default): skip 시군구 already in the manifest — resumable backfill.
      - "refresh": re-fetch **every** requested 시군구, overwriting its parquet and
        updating the manifest. This is the quarterly-refresh path (소상공인 상가 data
        is republished each 분기); write_region_parquet overwrites cleanly and the
        manifest keeps the newest row per region.
    """
    ...
    for region in regions:
        if mode != "refresh" and region in fetched:
            continue
        try:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="backfill",
        help="backfill: skip manifested 시군구; refresh: re-fetch all (quarterly refresh)",
    )
    args = parser.parse_args()
    df = extract(regions=args.regions, mode=args.mode)
    print(f"commercial: fetched {len(df)} rows this run ({args.mode})")
