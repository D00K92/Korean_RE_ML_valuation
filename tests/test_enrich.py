"""Tests for 청약홈 enrichment — schema + the leakage-guarded label join."""

from __future__ import annotations

import pandas as pd

from presale.features.enrich import aggregate_competition, enrich_labels
from presale.schemas.applyhome import ApplyhomeRecord


# -- schema: 전용 parsed from HOUSE_TY (floored), price in 만원 --------------
def test_area_parsed_from_house_type_not_supply_ar():
    r = ApplyhomeRecord(
        pblanc_no="2023000001", house_name="테스트자이", notice_date="2023-05-01",
        house_type="084.9600A", exclusive_area_m2="084.9600A", supply_price_manwon="70,000",
    )
...
def _applyhome():
    return pd.DataFrame(
        [
            ...
            dict(pblanc_no="A1", house_name="한강메트로자이(2단지)", supply_region="경기",
                 exclusive_area_m2=59.9, supply_price_manwon=60000,
                 notice_date=pd.Timestamp("2019-06-01"), move_in_ym="202112",
                 total_units=1000, builder="GS건설"),
            dict(pblanc_no="A1", house_name="한강메트로자이(2단지)", supply_region="경기",
                 exclusive_area_m2=84.9, supply_price_manwon=90000,
                 notice_date=pd.Timestamp("2019-06-01"), move_in_ym="202112",
                 total_units=1000, builder="GS건설"),
            dict(pblanc_no="A2", house_name="한강메트로자이", supply_region="경기",
                 exclusive_area_m2=84.9, supply_price_manwon=99999,
                 notice_date=pd.Timestamp("2023-01-01"), move_in_ym="202512",
                 total_units=1000, builder="GS건설"),
        ]
    )
...
def _competition():
    ...
    return pd.DataFrame(
        [
            dict(PBLANC_NO="A1", HOUSE_TY="084.9000", SUBSCRPT_RANK_CODE="1",
                 RESIDE_SENM="해당지역", REQ_CNT="200", SUPLY_HSHLDCO="10"),
            dict(PBLANC_NO="A1", HOUSE_TY="084.9000", SUBSCRPT_RANK_CODE="1",
                 RESIDE_SENM="기타지역", REQ_CNT="50", SUPLY_HSHLDCO="10"),
            dict(PBLANC_NO="A1", HOUSE_TY="059.9000", SUBSCRPT_RANK_CODE="1",
                 RESIDE_SENM="해당지역", REQ_CNT="8", SUPLY_HSHLDCO="20"),
        ]
    )
