...
from presale.extract.geocode import KEYWORD, PARCEL, ROAD, Coord, GeocodeCache, GeocodeClient
...
_MERGED_PARENT = {
    "41192": "41190", "41194": "41190", "41196": "41190",              # 부천시
    "41591": "41590", "41593": "41590", "41595": "41590", "41597": "41590",  # 화성시
}
...
# ---------------------------------------------------------------------------
def geocode_schools(
    df: pd.DataFrame, client: GeocodeClient, cache: GeocodeCache
) -> pd.DataFrame:
...
def geocode_apt(
    ...
    for sgg, dong, jibun, aptnm in keys.itertuples(index=False):
        ...
        resolved[(sgg, dong, jibun, aptnm)] = (
            coord.lat if coord else None, coord.lon if coord else None, prec
        )

    trip = out.apply(
        lambda r: resolved[(r["_sgg"], r["umdNm"], r["jibun"], r["aptNm"])], axis=1
    )
