...
from presale.extract import molit, molit_apt
...
_LEDGER_COLS = [
    "feed", "region", "deal_key",
    "first_seen_date", "last_seen_date", "cancelled_date", "rgst_filled_date",
]
...
def _normalize_label(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["deal_key"] = [
        _deal_key("label", r.region_code, r.complex_name, r.jibun,
                  r.exclusive_area_m2, r.floor, r.deal_date, r.price_manwon)
...
def _normalize_comps(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["deal_date"] = pd.to_datetime(
        dict(year=df["dealYear"].astype(int),
    ...
    out["deal_key"] = [
        ...
        for reg, dd, r in zip(out["region"], out["deal_date"], df.itertuples(index=False),
                              strict=False)
    ]
...
def _ledger_path(feed: str) -> Path:
    d = Path(get_settings().get("sources", "realtime", "ledger_dir",
                                default="data/realtime/seen_ledger"))
...
def update_ledger(feed: str, observed: pd.DataFrame, run_date: date) -> dict[str, int]:
    ...
    is_new = ~seen["deal_key"].isin(ledger["deal_key"]) if not ledger.empty else pd.Series(
        True, index=seen.index
    )
    ...
    appended = pd.DataFrame({
        ...
        "first_seen_date": pd.Series(ts, index=new_rows.index, dtype="datetime64[ns]").to_numpy(),
        "last_seen_date": pd.Series(ts, index=new_rows.index, dtype="datetime64[ns]").to_numpy(),
...
def write_audit(run_date: date, deltas: list[dict]) -> Path:
    ...
    delta_dir = Path(get_settings().get("sources", "realtime", "delta_dir",
                                        default="data/realtime/deltas"))
