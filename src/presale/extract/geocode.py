...
class GeocodeClient:
    ...
    def __init__(self) -> None:
        ...
        self._kakao_url = kk.get("keyword_url", "https://dapi.kakao.com/v2/local/search/keyword.json")
    ...
    def _vworld_search(self, query: str) -> Coord | None:
        ...
        params = {
            ...
            "crs": "EPSG:4326", "size": 5, "page": 1, "query": query,
            "type": "place", "format": "json", "key": self._vworld_key,
        }
        self._throttle()
...
class GeocodeCache:
        ...
        if self.path.exists():
            ...
            for row in df.itertuples(index=False):
                self._mem[(row.method, row.query)] = {
                    "status": row.status, "lat": row.lat, "lon": row.lon
                }
    ...
    def flush(self) -> None:
        ...
        rows = [
            {"method": m, "query": q, **rec}
            for (m, q), rec in self._mem.items()
        ]
